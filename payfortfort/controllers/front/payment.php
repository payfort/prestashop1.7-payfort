...
class PayfortfortPaymentModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if (isset($fortParams['action'])) {
            ...
            elseif($fortParams['action'] == 'displayError') {
                $this->displayError($this->getError());
            }
        }
    }
    ...
    public function postPaymentForm()
    {
        ...
        $isInstallments  = isset($_GET['INSTALLMENTS']) && $_GET['INSTALLMENTS'] == '1' ? true : false;
        $isSADAD         = isset($_GET['SADAD']) && $_GET['SADAD'] == '1' ? true : false;
        $isNaps          = isset($_GET['NAPS']) && $_GET['NAPS'] == '1' ? true : false;
        if ($isInstallments) {
            ...
            if (!Validate::isLoadedObject($customer)) {
                ...
                if ($success) {
                    ...
                    if($state == $cancelStatus) {
                        $this->setError($this->module->l('You have canceled the payment, please try agian.'));
                    }
                    else{
                        $supportLink = $this->module->l("customer support");
                        $this->setError($this->module->l('Sorry, Could not complete payment for your order, please check your payment details and try again. If you think this is an error, you can contact our '. $supportLink));
                    }
                    $redirectUrl = Context::getContext()->link->getPageLink('payment&fc=module&module=' .  $this->module->name . '&action=displayError');
                }
            }
    ...
    function merchantPageCancel()
    {
        ...
        $this->setError($this->module->l('You have canceled the payment, please try agian.'));
        Tools::redirect('index.php?fc=module&module=' .  $this->module->name . '&controller=payment&action=displayError');
    }
    
    private function setError($error_message) {
        $_SESSION['payfort_error_message'] = $error_message;
    }
    
    private function getError() {
        return $_SESSION['payfort_error_message'];
    }
