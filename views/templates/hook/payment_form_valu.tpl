...
<form style="" name="amazonpaymentservices_form" id={$payment_method|escape:'htmlall':'UTF-8'|cat:"_form"} class="aps_redirect_payment_form" method="post">
	...
	<div id="request_otp_sec" class="valu_form active">
		{if $allow_downpayment }
			<div class="valu-amount">
				<div class="aps aps-pad-none">
					<label>{l s='Enter valU downpayment amount (Optional):' mod='amazonpaymentservices'}</label>
					<input type="text" value="{$downpayment_value|escape:'htmlall':'UTF-8'}" autocomplete="off" maxlength="6" placeholder="Amount" value="0" class="form-control input-text valu_amount_field aps_valu_downpayment onlynum" />
				</div>
			</div>
		{/if}
		<div class="valu-amount">
			<div class="aps aps-pad-none">
				<label>{l s='Enter valu Wallet amount (Optional):' mod='amazonpaymentservices'}</label>
				<input type="text" value="" autocomplete="off" maxlength="6" placeholder="Amount" value="0" class="form-control input-text valu_amount_field aps_valu_wallet_amount onlynum" />
			</div>
		</div>
		<div class="valu-amount">
			<div class="aps aps-pad-none">
				<label>{l s='Enter valU cashback amount (Optional):' mod='amazonpaymentservices'}</label>
				<input type="text" value="" autocomplete="off" maxlength="6" placeholder="Amount" value="0" class="form-control input-text valu_amount_field aps_valu_cashback onlynum" />
			</div>
		</div>
	    <div class="aps-row valu-mobile">
	        <div class="aps col-sm-1 aps-pad-none">
	            <span class="country_code">{$country_code|escape:'htmlall':'UTF-8'}</span>
	        </div>
	        <div class="aps col-sm-6 aps-pad-none">
	            <input type="text" value="" autocomplete="off" maxlength="19" placeholder="{l s='Enter your mobile number' mod='amazonpaymentservices'}" class="form-control input-text aps_valu_mob_number onlynum" />
	        </div>
			<div class="aps col-sm-offset-1 col-sm-4">
